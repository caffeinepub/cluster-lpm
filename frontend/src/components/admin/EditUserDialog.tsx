...
export function EditUserDialog({ open, onOpenChange, userId, userProfile }: EditUserDialogProps) {
  ...
  useEffect(() => {
    if (open && userProfile) {
      setFormData({
        ...
        hotelId: userProfile.hotelId ? userProfile.hotelId.toString() : NO_HOTEL_VALUE,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const hotelId = formData.hotelId && formData.hotelId !== NO_HOTEL_VALUE
        ? BigInt(formData.hotelId)
        : null;
